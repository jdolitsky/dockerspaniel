...
ds.generateContents = A(function(spaniel, tags, callback, is_child, root_dir) {
    ...
    async.eachSeries(spaniel.steps, function(step, done) {
        ...
        // if any tag in 'unless_one' array is present, do not make step
        if (step.unless_one && step.unless_one.length) {
            skip = false;
            for (j=0; j<step.unless_one.length; j++) {
                if (tag_obj[step.unless_one[j]]) {
                    skip = true;
                    break;
                }
            }
        ...
        // if every tag in 'unless' array is present, do not make step
        ...
        // only make step if any tag in 'only_one' array is present
        if (step.only_one && step.only_one.length) {
            skip = true;
            for (j=0; j<step.only_one.length; j++) {
                if (tag_obj[step.only_one[j]]) {
                    skip = false;
                    break;
                }
            }
        ...
        // only make step if every tag in 'only' array is present
